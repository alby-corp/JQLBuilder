...
{
    internal static Bool Contains<T>(this IJqlField<T> left, T right) where T : IJqlLike<T> =>
        new BinaryOperator(left, Operators.Contains, right, Priority.Equality);

    internal static Bool NotContains<T>(this IJqlField<T> left, T right) where T : IJqlLike<T> =>
