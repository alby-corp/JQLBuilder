...
public static class HistoricalExtensions
{
    ...
    public static Bool WasIn<T>(this T left, IJqlCollection<T> right) where T : IJqlHistorical<T> => new BinaryOperator(left, "was in", right, Priority.Powerful);
    ...
    public static Bool WasNotIn<T>(this T left, IJqlCollection<T> right) where T : IJqlHistorical<T> => new BinaryOperator(left, "was not in", right, Priority.Powerful);
